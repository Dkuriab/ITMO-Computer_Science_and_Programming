...
public class F {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			treap.insert(new Node(scan.nextInt(), t), i + 1);
		}

		for (int i = 0; i < m; i++) {
			String[] q = in.readLine().split(" ");
			if (q[0].equals("del")) {
				treap.delete(treap.root, Integer.parseInt(q[1]), 0);
			} else {
				treap.insert(new Node(Integer.parseInt(q[2]), rm.nextInt(2000000)), Integer.parseInt(q[1]));
			}
		}

		int hm = treap.root == null ? 0 : treap.root.size;
		out.write(hm + "");
		out.newLine();
